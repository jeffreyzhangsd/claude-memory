...
- [Trading module (Form 4 + 13F + congress → hermes → Alpaca paper)](project_trading.md) — Shipped 2026-05-27. Local-first scorer (hermes3:8b), DeepSeek-reasoner escalation, ≤$50 auto-buy + $500/mo budget, 3 launchd daemons, dashboard tab + per-row paper-buy at `/api/trading/place-buy`. Owner added to dispatch. **2026-05-28: BOTH caps (monthly + per-trade) removed; auto vs manual P&L split via purchase_type field.**
...
- [Plan: Claude Pro + DeepSeek v4-pro orchestrator](plan_claude_pro_deepseek_orchestrator.md) — Downgrading Max → Pro; Claude orchestrates, DeepSeek v4-pro handles heavy reasoning via API. Key not yet in `.env`. Told Air, now Mini.
